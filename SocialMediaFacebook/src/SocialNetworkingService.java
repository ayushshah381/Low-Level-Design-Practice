...
public class SocialNetworkingService {
    ...
    public void sendFriendRequest(String fromId, String toId) {
        User fromUser = usersMap.get(fromId);
        User toUser = usersMap.get(toId);

        if(fromUser == null || toUser == null) throw new IllegalArgumentException("No user found");
        if(fromUser.isFriend(toUser)) return;
        FriendRequest req = new FriendRequest(fromUser, toUser);
        toUser.receiveRequest(req);
        sendNotification(toUser, NotificationType.FRIEND_REQUEST_ACCEPTED, "Friend request from " + fromUser.getName());
    }
