...
public class RideService {
    ...
    public void requestRide(Passenger passenger, Location source, Location destination) {
        Ride newRide = new Ride(generateRideId(), null, source, destination, RideStatus.REQUESTED, 0.0, passenger);
        requestedRides.offer(newRide);
        // notify the drivers about the newly requested ride
    ...
    public void notifyDrivers(Ride ride) {
        for(Driver driver: drivers.values()) {
            if(driver.getDriverStatus() == DriverStatus.AVAILABLE) {
                ...
                if(distance <= 5.0) {
                    // Notify the nearby drivers about the requested ride
                    System.out.println("Notifying driver: " + driver.getName() + " about ride request: " + ride);
                }
            }
        }
    }
